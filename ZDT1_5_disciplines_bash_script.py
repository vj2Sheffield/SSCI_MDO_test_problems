...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 16 16:43:21 2022
...
path = "[Run " + str(i) + "] DV 5 disciplines"
# Set parameters
N = 5
...
x5_dummy = np.ones((n_x4, 1))
y5_dummy = np.ones((n_y4, 1))
...
bArray = np.array([[ 4,  5],
       ...
       [ 5,  5],
       [10, 10]])
dArray = np.array([[ 9,  4,  4,  7,  3,  7,  3,  5,  5,  2,  4,  5],
       [ 2,  7,  8,  9,  7,  7,  1,  6,  6,  6,  1,  6],
       [ 1,  8,  6,  2,  4,  4,  3,  6,  1,  2,  3,  6],
       [ 8,  3,  4, 10,  2,  4,  8,  4,  4,  5,  1,  8],
       [ 1,  9,  3,  8,  1, 10,  5,  7,  5,  5,  3,  8],
       [ 9,  6,  5,  4,  6,  4,  9,  3,  8,  9,  8,  4],
       [ 4,  5,  2,  1,  9,  4,  6,  8,  9, 10,  5,  3],
       [ 1,  1,  7,  9,  8,  3,  7, 10,  2,  3,  7,  7],
       [ 8,  1,  8, 10,  5,  4,  9,  4,  7,  1,  7,  1],
       [10,  2,  2,  2,  4,  7,  4,  9,  6,  4,  2,  3]])
...
class disc1(om.ExplicitComponent):
	def setup(self):
		...
		self.add_input("y5", val = y2_dummy)
...
class MDA(om.Group):
	def setup(self):
		...
		cycle.add_subsystem("d1", disc1(), promotes_inputs = ["z", "y5", "x1"], promotes_outputs = ["y1"])
		...
		self.add_subsystem("obj2", om.ExecComp("f2 = (1 + (9/29)*(sum(x1) + sum(x2) + sum(x3) + sum(x4) + sum(x5) + sum(y1) + sum(y2) + sum(y3) + sum(y4) + sum(y5) + sum(z) - z[0]))*(1 - (z[0]/(1 + (9/29)*(sum(x1) + sum(x2) + sum(x3) + sum(x4) + sum(x5) + sum(y1) + sum(y2) + sum(y3) + sum(y4) + sum(y5) + sum(z) - z[0])))**0.5)",
                                            x1 = x1_dummy, x2 = x2_dummy, x3 = x3_dummy, x4 = x4_dummy, x5 = x5_dummy, z = z_dummy, y1 = y1_dummy, y2 = y2_dummy, y3 = y3_dummy, y4 = y4_dummy, y5 = y5_dummy), 
                     promotes = ["x1", "x2", "x3", "x4", "x5", "y1", "y2", "y3", "y4", "y5", "z", "f2"])
